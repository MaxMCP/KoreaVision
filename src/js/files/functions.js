...
/* Выборка одного чекбокса */
export function oneChoiceCheckBox() {
	$("input:checkbox").on('click', function () {
		// в обработчике "это" относится к окну, на которое нажали
		var $box = $(this);
		if ($box.is(":checked")) {
			// имя поля извлекается с помощью метода .attr()
			// поскольку это предполагается и ожидается неизменным
			var group = "input:checkbox[name='" + $box.attr("name") + "']";
			// отмеченное состояние группы / поля, с другой стороны, изменится
